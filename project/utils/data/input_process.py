...
def parse_id(id_):
    ...
    print(f"Num missing evals before: {np.sum(np.isnan(evals))}")
    evals = (np.array(evals) - mean) / std
    print(f"Num missing evals after: {np.sum(np.isnan(evals))}")
    shp = evals.shape
    ...
    """print(f"values shape final:{values.shape}")
    print(f"masks shape final:{masks.shape}")
    print(f"evals shape final:{evals.shape}")
    print(f"eval_masks shape final:{eval_masks.shape}")

    print(f'non_zero masks: {np.count_nonzero(masks)}')
    print(f'non_zero evals: {np.count_nonzero((~np.isnan(evals)))}')
    print(f'non_zero eval_masks: {np.count_nonzero(eval_masks)}')"""
